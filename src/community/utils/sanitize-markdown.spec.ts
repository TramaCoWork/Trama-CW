...
describe('sanitizeMarkdown', () => {
  it('keeps pure markdown unchanged', () => {
    ...
    expect(sanitizeMarkdown('[link](https://example.com)')).toBe('[link](https://example.com)');
    expect(sanitizeMarkdown('![img](/uploads/x.png)')).toBe('![img](/uploads/x.png)');
  });

  it('removes script tags and content', () => {
    expect(sanitizeMarkdown('<script>alert(1)</script>**bold**')).toBe('**bold**');
  });
