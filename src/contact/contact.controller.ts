...
export class ContactController {
  ...
  @ApiOperation({
    summary: 'Enviar formulario de contacto (publico, con captcha)',
  })
  @ApiBody({ type: CreateContactDto })
  @ApiResponse({ status: 201, description: 'Mensaje enviado exitosamente' })
  @ApiResponse({
    status: 400,
    description: 'Captcha invalido o servicio no configurado',
  })
  @ApiResponse({
    status: 429,
  ...
  @Throttle({ default: { ttl: 60000, limit: 5 } })
  @ApiOperation({
    summary: 'Enviar contacto a un profesional (publico, con captcha)',
  })
  @ApiBody({ type: CreateProfessionalContactDto })
  @ApiResponse({ status: 201, description: 'Mensaje enviado exitosamente' })
  @ApiResponse({
    status: 400,
    description: 'Captcha invalido o datos invalidos',
  })
