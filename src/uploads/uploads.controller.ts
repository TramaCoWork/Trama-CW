...
import {
  createPhotoFileValidationPipe,
  PHOTO_MAX_FILE_SIZE,
} from './photo-file-validation';
...
export class UploadsController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Subir un documento (DNI, CV, titulo, certificado)',
  })
    ...
    schema: {
      ...
      properties: {
        file: {
          type: 'string',
          format: 'binary',
          description: 'Archivo (PDF, JPG o PNG, max 5MB)',
        },
        type: {
          type: 'string',
          enum: ['dni', 'cv', 'title', 'certificate'],
          description: 'Tipo de documento',
        },
        educationId: {
          type: 'string',
          format: 'uuid',
          description: 'ID de educacion asociada (opcional)',
        },
        certificationId: {
          type: 'string',
          format: 'uuid',
          description: 'ID de certificacion asociada (opcional)',
        },
        professionId: {
          type: 'integer',
          description: 'ID de la profesion asociada (nivel 3, opcional)',
        },
      },
    },
  })
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Descargar un documento por ID (solo dueno o admin)',
  })
  @ApiResponse({ status: 200, description: 'Archivo descargado' })
  @ApiResponse({ status: 401, description: 'No autenticado' })
  @ApiResponse({
    status: 403,
    description: 'No tiene permiso para ver este documento',
  })
  ...
  async getDocument(
    ...
    const { absolutePath, document } =
      await this.uploadsService.getDocumentFile(id, user.userId, user.roles);
  ...
  @ApiBearerAuth()
  @UseInterceptors(
    FileInterceptor('file', { limits: { fileSize: PHOTO_MAX_FILE_SIZE } }),
  )
  ...
  @Get('photo/:profileId')
  @ApiOperation({
    summary: 'Obtener foto de perfil de un profesional (publico)',
  })
  ...
  async getPhoto(
    ...
    const { absolutePath, contentType } =
      await this.uploadsService.getPhotoFile(profileId);
  ...
  async deletePhoto(@CurrentUser() user: CurrentUserType) {
    ...
    schema: {
      ...
      properties: {
        front: {
          type: 'string',
          format: 'binary',
          description: 'Imagen frente del DNI (PDF, JPG o PNG, max 5MB)',
        },
        back: {
          type: 'string',
          format: 'binary',
          description: 'Imagen dorso del DNI (PDF, JPG o PNG, max 5MB)',
        },
      },
    },
  })
  @ApiResponse({
    status: 201,
    description: 'Documentos de identidad subidos exitosamente',
  })
  ...
  @ApiOperation({
    summary:
      'Obtener info de documentos de identidad de un profesional (admin)',
  })
  @ApiResponse({
    status: 200,
    description: 'Datos del documento de identidad (dni, URLs frente y dorso)',
  })
  @ApiResponse({ status: 404, description: 'Perfil no encontrado' })
  async getIdentityFiles(@Param('profileId', ParseUUIDPipe) profileId: string) {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Descargar imagen de identidad frente o dorso (admin)',
  })
  ...
  async getIdentityFile(
    ...
    const { absolutePath, contentType } =
      await this.uploadsService.getIdentityFile(profileId, side);
    res.setHeader('Content-Type', contentType);
    res.sendFile(absolutePath);
  }
}
