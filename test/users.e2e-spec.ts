...
describe('Users soft-delete (e2e)', () => {
  ...
  it('admin can list deleted users and restore them', async () => {
    ...
    expect(
      deletedUsers.body.data.some((u: { id: string }) => u.id === userId),
    ).toBe(true);
  ...
  it('excludes soft-deleted profile from public search', async () => {
    ...
    expect(
      beforeDelete.body.some((p: { id: string }) => p.id === profile!.id),
    ).toBe(true);
    ...
    expect(
      afterDelete.body.some((p: { id: string }) => p.id === profile!.id),
    ).toBe(false);
  });
});
